...
async function unlockDepartment(targetDept){
  ...
  try{
    const r=await fetch('/api/department-auth',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({department:targetDept,pin:String(pin).trim()})});
...
document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>enterDepartment(b.dataset.dept));
...
window.act=async(id,action)=>{if(['lost_one','damaged_one'].includes(action)&&!confirm('Xác nhận thao tác này?'))return;const r=await fetch(`/api/loans/${id}`,{method:'PATCH',headers:{'content-type':'application/json','x-admin-pin':departmentPin},body:JSON.stringify({action})});const j=await r.json();if(!r.ok)return alert(j.error||'Có lỗi');loadRows()};
window.delLoan=async id=>{if(!confirm('Chỉ xóa khi phiếu tạo sai. Xác nhận xóa?'))return;const r=await fetch(`/api/loans/${id}`,{method:'DELETE',headers:{'x-admin-pin':departmentPin}});const j=await r.json();if(!r.ok)return alert(j.error||'Có lỗi');loadRows()};
window.printLoan=async id=>{const r=await fetch(`/api/loans/${id}`,{headers:{'x-admin-pin':departmentPin}});const j=await r.json();if(!r.ok)return alert(j.error||'Không mở được phiếu');lastReceipt=j.row;renderReceipt(lastReceipt,'vi');$('#receiptSection').classList.remove('hidden');$('#receiptSection').scrollIntoView({behavior:'smooth'})};
$('#checkoutBtn').onclick=async()=>{const room=$('#checkoutRoom').value.trim(),pin=$('#checkoutPin').value.trim();if(!room||!pin)return alert('Nhập số phòng và PIN quản lý');const r=await fetch(`/api/checkout?room=${encodeURIComponent(room)}&department=${encodeURIComponent(dept)}`,{headers:{'x-admin-pin':pin}});const j=await r.json();if(!r.ok)return alert(j.error||'Không kiểm tra được');if(j.clear){$('#checkoutResult').innerHTML=`<div class="check-ok"><b>✓ PHÒNG ${esc(room)}: KHÔNG CÒN ĐỒ CHƯA TRẢ</b><br>Có thể tiếp tục quy trình checkout.</div>`}else{$('#checkoutResult').innerHTML=`<div class="check-alert"><b>⚠ PHÒNG ${esc(room)} CÒN ${j.open.reduce((a,x)=>a+x.remaining_qty,0)} MÓN CHƯA THU HỒI</b><div class="check-list">${j.open.map(x=>`<div class="check-item">${x.image_url?`<img src="${esc(x.image_url)}" class="checkout-thumb" alt="Ảnh">`:''}<span><b>${esc(deptName(x.department))}</b> · ${esc(x.item_name)} · còn <b>${x.remaining_qty}</b> · mã ${esc(x.code)}</span></div>`).join('')}</div></div>`}};
const _main=document.querySelector('main'); if(_main)_main.style.display='none';
enterDepartment('fb');
